...
def index():
    ...
    # unimos las dos tablas con JOIN, asignamos un alias a cada una t para todo u para user e indicamos que el campo created_by de la tabla todo (t) tiene que ser igual al ide de la tabla user (u). Ordenamos por created_by en order desc

    # indicamos las columnas que nos queremos traer de las DOS tablas justo después del select
...
def update(id):
    ...
    if request.method == 'POST':
        ...
        if not description:
              error = 'Descripción del todo requerida'
